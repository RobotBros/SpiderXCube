...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;							//PA5-CE 推挽 	  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	
    
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;						//PB12-CSN 推挽 	  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
    
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_4;   
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 				//PA4-IRQ 输入  
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 
	GPIO_Init(GPIOA, &GPIO_InitStructure);
    
	GPIO_SetBits(GPIOA,GPIO_Pin_4|GPIO_Pin_5); 						//CE IRQ上拉,禁止NRF24L01工作		
	GPIO_ResetBits(GPIOB,GPIO_Pin_12); 										//CSN下拉
    
    SPI2_Init();    																		  //SPI2 initial   
	
	NRF24L01_CE=0; 																				//NRF24L01 enable
	NRF24L01_CSN=1;	
}
...
void NRF24L01_RX_Mode(void)
{
    NRF24L01_CE=0;	  
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+CONFIG, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 IRQ enable 
    SPI2_SetSpeed(SPI_BaudRatePrescaler_8); //spi设置为9Mhz (24L01 SPI时钟最大值为10Mhz)
	NRF24L01_Write_Reg(FLUSH_RX,0xff);//清空 RX FIFO
	NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,0xff);	//清空状态寄存器
...
{														 
    ...
    NRF24L01_Write_Reg(FLUSH_TX,0xff);//清空 TX FIFO
	NRF24L01_Write_Reg(FLUSH_RX,0xff);//清空 RX FIFO
	NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,0xff);	//清空状态寄存器
	NRF24L01_CE=1;//CE为高,10us后启动发送
}		  

////启动NRF24L01 IRQ intterupt RX数据
////rxbuf:待接收数据首地址
////返回值:0，接收完成；
////			:1, 错误
//u8 NRF24L01_RxPacket_IRQ(u8 *rxbuf)
//{
//	u8 sta;		    							     
//	sta=NRF24L01_Read_Reg(STATUS); 							 				  //读取状态寄存器的值    	 
//	NRF24L01_Write_Reg(WRITE_REG_NRF+STATUS,sta); 			  //清除TX_DS或MAX_RT中断标志
